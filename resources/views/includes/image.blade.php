@if(!empty($images))
    ...
    @foreach($images as $key)
        <!-- Button trigger modal -->
            <img data-toggle="modal" data-target="#{{ $key->id }}" src="/laravel/storage/app/{{ $key->name }}"
                 width="90px" height="90px" id="{{ $key->name }}">
            <!-- Modal -->
            <div class="modal fade" id="{{ $key->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-body">
                            <img src="/laravel/storage/app/{{ $key->name }}" width="690px" height="690px"
                                 id="{{ $key->name }}">
